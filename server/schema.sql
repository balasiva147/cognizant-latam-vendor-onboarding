...
CREATE TABLE IF NOT EXISTS onboarding_tickets (
  ...
  workflow_stage ENUM('VENDOR','LOCAL','TRANSLATION','INDIA','SECURITY','LOCAL_REWORK','COMPLETED') NOT NULL DEFAULT 'VENDOR',
...
CREATE TABLE IF NOT EXISTS ticket_documents (
  ...
  status ENUM('INIT','DOCUMENTS_UPLOADED','LOCAL_PROCUREMENT_ACCEPTED')
    NOT NULL DEFAULT 'INIT',
  india_status ENUM('PENDING','APPROVED','REJECTED') NOT NULL DEFAULT 'PENDING',
  security_status ENUM('PENDING','APPROVED','REJECTED') NOT NULL DEFAULT 'PENDING',
...
CREATE TABLE IF NOT EXISTS document_review_events (
  ...
  review_stage ENUM('LOCAL','INDIA','SECURITY') NOT NULL DEFAULT 'LOCAL',
  decision ENUM('LOCAL_PROCUREMENT_ACCEPTED','INDIA_PROCUREMENT_ACCEPTED','SECURITY_ACCEPTED','REJECTED') NOT NULL,
